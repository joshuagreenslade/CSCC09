...
app.post('/api/messages/', function (req, res, next) {
    ...
    //check for invalid username, upvote, downvote and escapes the content
    if((req.body.upvote !== undefined) || (req.body.downvote !== undefined)){
        res.status(400).end("upvote and downvote cannot be defined");
        return next();
    }
    req.checkBody('username', 'Invalid username').notEmpty().isAlphanumeric();
    if(req.validationErrors()){
...
app.get('/api/users/:username/picture/', function (req, res, next) {
    ...
    req.checkParams('username', 'Invalid username').notEmpty().isAlphanumeric();
    if(req.validationErrors()){
        res.status(400).end("Invalid username");
        return next();
    }
...
app.patch('/api/messages/:id/', function (req, res, next) {
    ...
    if((req.body.action !== "upvote") && (req.body.action !== "downvote")){
